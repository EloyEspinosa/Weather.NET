...
namespace Weather.NET
{
    ...
    public class CurrentWeather
    {
        ...
        {
            string file = await ReadWebpageAsync($"https://api.openweathermap.org/data/2.5/weather?q={cityName}&appid={apiKey}&units={measurement}&lang={language}");
            dynamic json = JsonConvert.DeserializeObject<dynamic>(file);

            try
            {
                var output = new CurrentWeather
                {
                    CityName = json.name,
                    CityId = json.id,
